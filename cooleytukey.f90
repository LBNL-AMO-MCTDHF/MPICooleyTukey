...
!! INVERSE OF cooleytukey_outofplace
...
subroutine cooleytukey_outofplace_inverse(intranspose,out,dim1,pf,howmany)
  ...
  complex*16 ::  intransconjg(dim1,pf(1)*OTHERMACRO,howmany), &
       outconjg(dim1,pf(1)*OTHERMACRO,howmany)

  intransconjg(:,:,:)=CONJG(intranspose(:,:,:))
  call cooleytukey_outofplaceinput(intransconjg,outconjg,dim1,pf,howmany)
  out(:,:,:)=CONJG(outconjg(:,:,:))/dim1/( pf(1)*OTHERMACRO )

end subroutine cooleytukey_outofplace_inverse



subroutine twiddlemult(in,out,dim1,dim2,howmany)
  implicit none
  integer, intent(in) :: dim1,dim2,howmany
  complex*16, intent(in) :: in(dim1,dim2,howmany)
  complex*16, intent(out) :: out(dim1,dim2,howmany)
  complex*16 :: twiddle1(dim1),tt1(dim1)  !! AUTOMATIC
  integer :: ii,n2

  call gettwiddlesmall(twiddle1(:),dim1,dim2)
  do n2=1,dim2
     tt1(:)=twiddle1(:)**(n2-1)
     do ii=1,howmany
        out(:,n2,ii) = in(:,n2,ii) * tt1(:)
     enddo
  enddo
end subroutine twiddlemult
...
recursive subroutine cooleytukey_outofplace(in,outtrans,dim1,pf,howmany)
  ...
  otherfactor= OTHERMACRO

  call myzfft1d_slowindex(in,tempout,dim1*otherfactor,pf(1),howmany)
  call twiddlemult(tempout,outtemp,dim1*otherfactor,pf(1),howmany)
...
!! OUT-OF-PLACE INPUT, most compact subroutine AND best for MPI
...
!! this will be the MPI subroutine

subroutine myzfft1d_slowindex(in,out,dim1,dim2,howmany)
  implicit none
  integer, intent(in) :: dim1,dim2,howmany
  complex*16, intent(in) :: in(dim1,dim2,howmany)
  complex*16, intent(out) :: out(dim1,dim2,howmany)
  ...
  do ii=1,howmany
     do jj=1,dim1
        call ZGEMV('N',dim2,dim2,(1d0,0d0),fouriermatrix,dim2,in(jj,1,ii),dim1,(0d0,0d0),out(jj,1,ii),dim1)
     enddo
  enddo
...
!!$  subroutine cooleytukey_inverse(in,out,dim1,dim2,howmany)
!!$    implicit none
!!$    integer, intent(in) :: dim1,dim2,howmany
!!$    complex*16, intent(in) :: in(dim1,dim2,howmany)
!!$    complex*16, intent(out) :: out(dim1,dim2,howmany)
!!$    complex*16 ::  intranspose(dim2,dim1,howmany),temp1(dim1,dim2,howmany) !!AUTOMATIC
!!$  
!!$    temp1(:,:,:)=CONJG(in(:,:,:))
!!$    call cooleytukey_replace(1,temp1,intranspose,dim1,dim2,howmany)
!!$    call cooleytukey_outofplaceinput(intranspose,temp1,dim2,dim1,howmany)
!!$    out(:,:,:)=CONJG(temp1(:,:,:))/dim1/dim2
!!$  
!!$  end subroutine cooleytukey_inverse
!!$  
!!$  
!!$  
!!$  !! fourier transform
!!$  
!!$  subroutine cooleytukey(in,out,dim1,dim2,howmany)
!!$    implicit none
!!$    integer, intent(in) :: dim1,dim2,howmany
!!$    complex*16, intent(in) :: in(dim1,dim2,howmany)
!!$    complex*16, intent(out) :: out(dim1,dim2,howmany)
!!$    complex*16 :: outtrans(dim2,dim1,howmany)         !! AUTOMATIC
!!$  
!!$    call cooleytukey_outofplace(in,outtrans,dim1,dim2,howmany)
!!$    call cooleytukey_replace(1,outtrans,out,dim1,dim2,howmany)
!!$  
!!$  end subroutine cooleytukey
!!$  
!!$  
!!$  !! COMPUTE FT WITH COOLEY TUKEY, REORDERING FIRST
!!$  
!!$  subroutine cooleytukey2(in,out,dim1,dim2,howmany)
!!$    implicit none
!!$    integer, intent(in) :: dim1,dim2,howmany
!!$    complex*16, intent(in) :: in(dim1,dim2,howmany)
!!$    complex*16, intent(out) :: out(dim1,dim2,howmany)
!!$    complex*16 ::  intranspose(dim2,dim1,howmany)
!!$  
!!$    call cooleytukey_replace(1,in,intranspose,dim1,dim2,howmany)
!!$    call cooleytukey_outofplaceinput(intranspose,out,dim2,dim1,howmany)
!!$  
!!$  end subroutine cooleytukey2
!!$  
!!$  
